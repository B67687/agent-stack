...
export async function buildLanguageModel(
  ...
  switch (provider) {
    ...
    case "acp": {
      // ACP provider: uses `omp acp` via Tauri Rust backend (stdio JSON-RPC).
      // Streams responses in real-time via Tauri IPC Channel.
      const { invoke, Channel } = await import("@tauri-apps/api/core");

      // Ensure ACP is initialized
      const initResult: any = await invoke("acp_initialize");
      if (!initResult?.success) {
        throw new Error(initResult?.error || "ACP initialization failed");
      }

      // Get or create session (non-streaming)
      const sessionResult: any = await invoke("acp_send", {
        method: "session/new",
        params: { cwd: "/", mcpServers: [] },
      });
      const sessionId: string | undefined = sessionResult?.result?.sessionId;
          ...
          return {
            stream: new ReadableStream({
              start(controller: any) {
                // Create Tauri streaming channel
                const channel = new Channel<any>();
                channel.onmessage = (event: any) => {
                  if (event.kind === "chunk") {
                    // Parse the ACP update JSON and extract text
                    try {
                      const update = JSON.parse(event.text);
